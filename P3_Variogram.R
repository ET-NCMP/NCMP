...
#  Dl[nbin] <- ((max(Dmax,Dmax1)+bin[nbin-1])/2)+w  # Halfway to maximum actual distance
...
# nls fits Bl to function(Dl,a,b,s) by changing parameters a and b and fixing s   #
    mod1 <- nls(Bl~Gaussian(Dl,a,b,s),start=list(a=1,b=600),                      #
       control=nls.control(warnOnly=TRUE,minFactor=0.0001,maxiter=100))           #
    cmod1 <- coef(mod1)                                                           #
                                                                                  #
    mod2 <- nls(Bl~Exponential(Dl,a,b,s),start=list(a=0,b=400),                   #
       control=nls.control(warnOnly=TRUE,maxiter=100))                            #
    cmod2 <- coef(mod2)                                                           #
                                                                                  #
    if (nm != 13L) {                                                              #
      mod3 <- nls(Bl~Spherical(Dl,a,b,s),start=list(a=0,b=800),                   #
        control=nls.control(warnOnly=TRUE,maxiter=100))                           #
      cmod3 <- coef(mod3) }                                                       #
    else cmod3 <- rep(NA,2)                                                       #
                                                                                  #
    n <- c(cmod1[1],cmod2[1],cmod3[1])           # Copy fitted a to coefficient n #
    r <- c(cmod1[2],cmod2[2],cmod3[2])           # Copy fitted b to coefficient r #
                                                                                  #
# Calculate mean squared error for each fit                                       #
                                                                                  #
    E <- rep(Inf,3)      # Initial value of infinity for non-spherical for annual #
    E[1] <- sum((Bl - Gaussian(Dl,n[1],r[1],s))^2,na.rm=TRUE)        # vectorised #
    E[2] <- sum((Bl - Exponential(Dl,n[2],r[2],s))^2,na.rm=TRUE)                  #
    if (nm != 13L) {                                                              #
      E[3] <- sum((Bl - Spherical(Dl,n[3],r[3],s))^2,na.rm=TRUE) }                #
                                                                                  #
# Keep name and parameters of best fit                                            #
    ...
    X[nm,"Mean Sq Err"] <- E[k]                                                   #
                                                                                  #
    curve(get(Graph[k])(x,n[k],r[k],s),col="red",add=TRUE) # Add best fit to plot #
                                                                                  #
