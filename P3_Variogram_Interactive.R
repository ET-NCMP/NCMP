...
cat("***** P3_Variogram_Interactive.R *****",fill=TRUE)
...
cat("Calculating Variograms from monthly and annual station series for selected NCMP indices",
    "This version is interactive by allowing a user-defined maximum station distance",sep="\n")
...
# Enure that using correct maximum distance for temperature or precipitation
...
# Have commented out rounding of the mean binned value
  ...
  Bl_full <- rep(NA,nbin)  # empty vector for bin avgs     
...
# For iterative loop, set bin values to missing for bins where the distance
# is greater than the current maximum range
# TEST: Add half bin width to ensure outer bin is captured on first pass
...
# Highlight best fit in red
    ...
    response <- readline('Happy with variogram? (y/n)')
    if (response == 'y') { 
      fixed_range <- FALSE 
    } else {
	  maxrange <- NA_real_
	  while (is.na(maxrange) || maxrange < 0 || maxrange > Dmax) {
        maxrange <- readline("Enter new maximum range for variogram : ")
        maxrange <- suppressWarnings(as.numeric(maxrange))
	  }
    }
  }
}
