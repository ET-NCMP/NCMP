...
# - need to reset the diagonal (same station) to zero
...
# No need to do this if do not have any data in the bin, but account for missing
      ...
      for (k in which(nrec > 0L)) {                   # Loop through non-empty bins
        ...
        Bl_full[k] <- mean(b2,na.rm=TRUE)                        # assign into vector Bl
	if (Dl[k] > maxrange) { Bl[k] <- NA }
      }                                                              # End bin loop
...
#highlight best fit in red
      ...
      if (response == 'y') { 
        ...
        maxrange <- readline('Enter new maxrange:')
        maxrange <- suppressWarnings(as.integer(maxrange))
      }
